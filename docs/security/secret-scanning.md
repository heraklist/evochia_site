...
The blocking security workflow scans the complete Git history, including the current checked-out commit, with credential-free TruffleHog OSS. The checkout is mounted read-only in a read-only container; only an isolated /tmp tmpfs is writable. TruffleHog reports both verified and unknown results, fails on either class, fails on scan errors, and cannot self-update. The pinned --github-actions formatter reports detector, status, and source location without printing raw secret values.
...
The formatter choice is also version-pinned security behavior. At commit 6f3c981e7b77f235fd2702dd74af25fc4b72bf11, the [plain printer](https://github.com/trufflesecurity/trufflehog/blob/6f3c981e7b77f235fd2702dd74af25fc4b72bf11/pkg/output/plain.go) copies r.Raw and prints it as Raw result. The [GitHub Actions printer](https://github.com/trufflesecurity/trufflehog/blob/6f3c981e7b77f235fd2702dd74af25fc4b72bf11/pkg/output/github_actions.go) formats only finding metadata and never reads r.Raw; the pinned [CLI wiring](https://github.com/trufflesecurity/trufflehog/blob/6f3c981e7b77f235fd2702dd74af25fc4b72bf11/main.go) selects that printer with --github-actions. The flag is therefore mandatory even when GitHub may mask known secret patterns.
