...
test("CSP stays consistent across vercel and middleware", () => {
  ...
  assert.match(vercelCsp, /form-action 'self' https:\/\/formspree\.io/);
...
test("localized 404 pages stay noindex and omit canonical and hreflang markup", () => {
  for (const locale of locales) {
    ...
    assert.match(html, /<script type="module" src="\/js\/cookieconsent-config\.js"><\/script>/);
  }
});

test("faq and lookbook pages keep the full localized navigation shell", () => {
  const expectations = [
    { file: pagePath("en", "faq"), switchLabel: "EN/EL" },
    { file: pagePath("en", "lookbook"), switchLabel: "EN/EL" },
    { file: pagePath("el", "faq"), switchLabel: "EL/EN" },
    { file: pagePath("el", "lookbook"), switchLabel: "EL/EN" },
  ];

  for (const expectation of expectations) {
    const html = readRepoFile(expectation.file);

    assert.match(html, /class="nav-logo-img"/, `${expectation.file} should render the shared nav logo image`);
    assert.match(
      html,
      /href="\/(?:en|el)\/about\/"[^>]+data-en="About"[^>]+data-el="Σχετικά"/,
      `${expectation.file} should keep localized nav data attributes`,
    );
    assert.match(
      html,
      new RegExp(`class="lang-switch"[\\s\\S]*?>${escapeRegex(expectation.switchLabel)}<`),
      `${expectation.file} should expose the correct language switch label`,
    );
    assert.doesNotMatch(
      html,
      /role="menubar"|role="menuitem"/,
      `${expectation.file} should use standard nav semantics`,
    );
  }
});

test("localized footers keep FAQ and Lookbook links discoverable", () => {
  for (const locale of locales) {
    for (const page of contentPages) {
      const html = readRepoFile(pagePath(locale, page));
      const faqHref = locale === "en" ? "/en/faq/" : "/el/faq/";
      const lookbookHref = locale === "en" ? "/en/lookbook/" : "/el/lookbook/";

      assert.match(
        html,
        new RegExp(`<a href="${escapeRegex(faqHref)}"[^>]*>`),
        `${pagePath(locale, page)} should expose a footer FAQ link`,
      );
      assert.match(
        html,
        new RegExp(`<a href="${escapeRegex(lookbookHref)}"[^>]*>`),
        `${pagePath(locale, page)} should expose a footer Lookbook link`,
      );
    }
  }
});
